...
fn read_cnf_file(srcf:&PathBuf)->Result<FxLinkedHashMap<String,FxHashMap<String,String>>,Error>{
let file= File::open(srcf)?;
read_cnf(&mut BufReader::new(file).lines())
}  
fn read_cnf<I:BufRead>(lte:&mut Lines<I>)->Result<FxLinkedHashMap<String,FxHashMap<String,String>>,Error>{
    ...
    let mut var=linev.trim();
     ...
     let mut j:usize=0;
        while j<usize::MAX {
            ...
            if !skip {strbuf.push_str(&var[j..k]);}
            j=m;
        }
        if ispix {
            match lte.next() {
     ...
     } else if let Some(ref key) =lastkey{
         let list:Vec<&str> =var.splitn(2,|c|c=='.'||c=='=').collect();
         if list.len()>1 {
             if let None=last{
             last=Some(table.entry(key.to_string()).or_insert(FxHashMap::default()));
             }
...
fn write_cnf_file(drcf:&PathBuf,map:&FxLinkedHashMap<String,FxHashMap<String,String>>)->Option<Error>{
let f=get_some_err!(File::open(drcf));
write_cnf_io(&mut BufWriter::new(f),map)
}
fn write_cnf_io<W:Write>(buf:&mut BufWriter<W>,map:&FxLinkedHashMap<String,FxHashMap<String,String>>)->Option<Error>{
let mut ishead=false;
for (k,v) in map{
  if v.len()==0 {continue;}
  if ishead {some_err!(buf.write(&[b'\n']));}
  ishead=true;
